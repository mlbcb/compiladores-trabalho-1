...
matched_stmt
    : IF expr THEN matched_stmt ELSE matched_stmt END IF SEMICOLON { $$ = mk_if($2, $4, $6); }
    | WHILE expr LOOP stmt_lst END LOOP SEMICOLON                  { $$ = mk_while($2, $4); }
    | ID ASSIGN expr SEMICOLON                                     { $$ = mk_assign($1, $3); }
    ;
/* Unmatched statements: have dangling parts that can absorb an ELSE */
unmatched_stmt
    : IF expr THEN stmt END IF SEMICOLON                           { $$ = mk_if($2, $4, NULL); }
    | IF expr THEN matched_stmt ELSE unmatched_stmt END IF SEMICOLON { $$ = mk_if($2, $4, $6); }
    ;
expr
    : NUM                         { $$ = mk_numexp($1); }
    | REAL                        { $$ = mk_floatexp($1); }
